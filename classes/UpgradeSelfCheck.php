...
class UpgradeSelfCheck
{
    ...
    public function phpCompatibleVersions(string $targetPrestashopVersion = null)
    {
        ...
        $prestashopVersionsList = array_keys(self::PHP_PS_VERSIONS);
        $previousVersion = reset($prestashopVersionsList);
        foreach (self::PHP_PS_VERSIONS as $prestashopVersion => $phpVersions) {
            // We reached a version higher than upgrade version, we get out of the loop
            if (version_compare($targetPrestashopVersion, $prestashopVersion, '<')) {
                break;
            }
            $previousVersion = $prestashopVersion;
        }

        return self::PHP_PS_VERSIONS[$previousVersion];
    }
